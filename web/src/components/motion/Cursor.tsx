 ...
 *  раскрывается, при нажатии сжимается. Рисуется через mix-blend-mode: difference, поэтому
 *  одинаково читается на светлом фоне и поверх тёмного видео. Работает только с мышью:
 *  на тач-устройствах и при prefers-reduced-motion компонент ничего не рисует. */
...
export function Cursor() {
  ...
  useEffect(() => {
    ...
    const onOver = (event: PointerEvent) => {
      ...
      gsap.to(ring, {
        scale: target ? 1.9 : 1,
        borderColor: target ? "rgba(255,255,255,1)" : "rgba(255,255,255,0.7)",
        duration: 0.45,
