...
def _about_method(seasons: list[dict]) -> str:
    """Как получен ряд поля.

    Обученная модель (LightGBM + сезонная нейросеть) восстанавливает скрытые точки только там, где они
    есть, — на полях кейса. Для территории, собранной сервисом, кривая сезона строится сглаживанием
    приведённых к одной шкале снимков, и обещать здесь работу модели нельзя.
    """
    restored = sum(int(s.get("восстановлено_точек") or 0) for s in seasons)
    how = ("пропуски в контрольных точках восстановлены обученной моделью (LightGBM и сезонная нейросеть)"
           if restored else
           "кривая сезона построена сглаживанием всех снимков, приведённых к шкале Sentinel-2 "
           "(обученная модель восстановления работает на данных кейса, к собранным полям она не применяется)")
    return (f"Ряд зелёности собирается из снимков Sentinel-2, Landsat и MODIS, {how}, "
