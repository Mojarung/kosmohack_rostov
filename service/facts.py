...
def field_brief(detail: dict, year: int | None = None) -> dict:
    """Короткая выжимка для промпта модели: без ежедневных рядов.

    Ряд сезона занимает почти всю длину полных фактов (около 20 КБ), а модели он нужен редко:
    за подробностями у неё есть инструменты. Короткий контекст заметно ускоряет ответ.
    """
    facts = field_facts(detail, year)
    facts.pop("ряд_выбранного_сезона", None)
    facts["эпизоды"] = facts.get("эпизоды", [])[:5]
    facts["подсказка"] = ("Подробности по сезону, полному списку периодов снижения и методу "
                          "запрашивай инструментами season, episodes и method.")
    return facts
