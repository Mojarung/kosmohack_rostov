...
# Пишем JSON строго в UTF-8: без encoding на Windows файл уходил в cp1251 и ломал кириллицу
import os
from pathlib import Path
OUT_DIR = Path('reports/dashboard'); OUT_DIR.mkdir(parents=True, exist_ok=True)
data_path = OUT_DIR / 'dashboard_data.json'
with open(data_path, 'w', encoding='utf-8') as f:
    json.dump(out, f, ensure_ascii=False, separators=(',',':'))
print('polys', len(polys), 'groups', len(set(grp.values())), 'size MB', round(os.path.getsize(data_path)/1e6,2))
# Подставляем данные в шаблон и оборачиваем в полноценный HTML-документ, чтобы файл открывался через file://
template = (OUT_DIR / 'template.html').read_text(encoding='utf-8')
assert template.count('__DATA__') == 1
page = template.replace('__DATA__', data_path.read_text(encoding='utf-8'))
HEAD = ('<!doctype html>\n<html lang="ru">\n'
        '<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"></head>\n'
        '<body>\n')
TAIL = '\n</body>\n</html>\n'
(OUT_DIR / 'dashboard.html').write_text(HEAD + page + TAIL, encoding='utf-8')
print('dashboard:', OUT_DIR / 'dashboard.html', round(os.path.getsize(OUT_DIR / 'dashboard.html')/1e6,2), 'MB')
