...
Запуск: uv run python -m gapfill.predict --input data/test_features_new.csv --output submission.csv
        (дополнительно: --n-masks 30 --rounds 5500 --seeds 0 1 2 --clip -0.1 1.0)
Модель LightGBM обучается на train + известных точках входного файла и файлов --extra (n_masks масок),
контекст для контрольных точек (is_synthetic_gap = True) — все известные точки. Несколько seeds усредняются.
...
from gapfill.config import ARTIFACTS_DIR, EXTRA_PATHS, SUBMISSION_PATH, TARGET, TEST_PATH, TRAIN_PATH
