...
Разведочный анализ `train_dataset.csv` и `test_dataset.csv` под задачу восстановления `primary_ndvi` (задача 1, автометрика) и детекции аномалий (задача 2). Скрипты анализа лежат в [`../eda/v1/`](../eda/v1/), графики в [`../eda/v1/figures/`](../eda/v1/figures/).

> **Статус.** Это заметки первого прохода EDA из ветки `eda/data-analysis`. Все числа перепроверены запуском скриптов и независимым пересчётом (см. [10-branch-comparison.md](10-branch-comparison.md)); три уточнения внесены прямо в текст и помечены «уточнено». Итоговый отчёт с расширенным анализом — [08-eda-report.md](08-eda-report.md).
...
![sensor bias](../eda/v1/figures/sensor_bias.png)
...
- Сенсор скрытой точки можно угадывать по расписанию: MODIS всегда на doy ≡ 1 (mod 16), Landsat с шагом 8/16 дней от соседних Landsat-дат, S2 с шагом 5 дней. Простое правило угадывает сенсор в 81 % случаев (уточнено: Landsat почти безошибочно, MODIS в 79 % случаев, S2 в 60 % — путается с Landsat; правило из `eda/cycles.py` даёт 92 %). В test в строках-гэпах сенсорные колонки затёрты, но даты соседних наблюдений полигона доступны, так что этот трюк применим.

![baseline errors](../eda/v1/figures/baseline_errors.png)
...
![test gaps by year](../eda/v1/figures/test_gaps_by_year.png)

### 2.5. Train-полигоны в test: утечки нет, но норма переносится

Пересечение по `anon_polygon_id` полное (39 из 39), но по датам пересечения нет: в test для train-полигонов есть только сезон 2025. Прямого копирования ответов не будет. Зато для этих 39 полигонов у нас есть 15 лет истории и климатическая норма из train, а 19 новых полигонов в test имеют историю 2010–2025 с известными `primary_ndvi` (уточнено: 12 с ежедневной сеткой и 7 без неё), то есть норму для них можно посчитать из самого test.

Остальные 20 новых полигонов (уточнено) представлены **только сезоном 2025** (213 или меньше строк). У них нет истории, и норму придётся брать из похожих полигонов или культуры. Это самый сложный сегмент.
...
ERA5 не уникален на полигон: у 29 train-полигонов с погодой (уточнено: у 10 разреженных её нет) всего **22 различных ряда** погоды, то есть соседние поля сидят в одной ячейке ERA5 (например AOI-0019…0022 в одной). Это можно использовать и для восстановления ERA5 в гэпах, и как признак пространственной близости полигонов.
...
![example polygon](../eda/v1/figures/example_polygon_2021.png)
...
![anomaly share by year](../eda/v1/figures/anomaly_share_by_year.png)
...
![seasonal profile](../eda/v1/figures/seasonal_profile_by_crop.png)
...
Из корня репозитория, окружение проекта (Python 3.14, pandas 3.0):

```bash
uv run python eda/v1/eda_01_overview.py   # размеры, NaN, полигоны (3 с)
uv run python eda/v1/eda_02_deep.py       # сенсоры, климатология, структура test (19 с)
uv run python eda/v1/eda_03_gaps.py       # ERA5, расписания сенсоров, baseline, соседи (72 с)
uv run python eda/v1/eda_04_signal.py     # сенсорно-осведомлённый baseline, сезонность, погода, графики (32 с)
uv run python eda/v1/eda_05_crop_check.py # проверка пика сезона по культурам (2 с)
uv run python eda/v1/build_dashboard_data.py  # данные + reports/dashboard/dashboard.html (23 с)
```
